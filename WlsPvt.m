function [xHat,z,svPos,H,Wpr,Wrr,pseudo_delays] = WlsPvt(prs,gpsEph,xo,pseudo_delays,y,iValid)
...
%----------------------------End of iteration v = line of sights; svXyzTrx = sat position


sats = size(v, 2);

P = [ -2699936.43; -4292566.69; 3855508.37];
zenith_delay = 3.08544;

for j = 1:sats
    incident_angle = acos(dot(P, -v(:,j))/(norm(P)*norm(-v(:,j))));
    elevation_angle = pi/2 - incident_angle;
    pseudo_delay = zenith_delay/sin(elevation_angle);
    pseudo_delays(y,iValid(j)) = pseudo_delay;
end
